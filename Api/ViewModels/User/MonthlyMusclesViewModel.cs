...
public class MonthlyMusclesViewModel
{
    ...
    /// </summary>
    public double MinSecsPerWeek = Math.Round(UserController.MuscleTargets.Values.Average(r => r.Start.Value));

    /// <summary>
    /// The avg maximum number of seconds per week each muscle group should be under tension.
    /// </summary>
    public double MaxSecsPerWeek = Math.Round(UserController.MuscleTargets.Values.Average(r => r.End.Value));

    // The max value (seconds of time-under-tension) of the range display
    public double MaxRangeValue = UserController.MuscleTargets.Values.Max(r => r.End.Value);

    public MonthlyMuscle GetMuscleTarget(MuscleGroups muscleGroup)
    {
        var userMuscleTarget = User.UserMuscles.Cast<UserMuscle?>().FirstOrDefault(um => um?.MuscleGroup == muscleGroup)?.Range ?? UserController.MuscleTargets[muscleGroup];
        var defaultMuscleTarget = UserController.MuscleTargets[muscleGroup];
