...
public class UserNewsletterDto(UserDto user, string token)
{
    ...
    //[JsonIgnore]
    public ICollection<UserExerciseDto> UserExercises { get; init; } = user.UserExercises;

    //[JsonIgnore]
    public ICollection<UserVariationDto> UserVariations { get; init; } = user.UserVariations;
