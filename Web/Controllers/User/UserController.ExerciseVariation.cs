...
public partial class UserController
{
    ...
    /// </summary>
    [HttpGet]
    [Route("{section:section}/{exerciseId}/{variationId}/s", Order = 1)]
    [Route("{section:section}/{exerciseId}/{variationId}/suspend", Order = 2)]
    public async Task<IActionResult> SuspendExercise(string email, string token, int exerciseId, int variationId, Section section)
    {
        var user = await userRepo.GetUser(email, token, allowDemoUser: true);
        if (user == null)
        {
