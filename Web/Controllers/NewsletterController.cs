...
public class NewsletterController : BaseController
{
    ...
    public async Task<IActionResult> Newsletter(string email, string token)
    {
        ...
        _context.AddMissing(await _context.UserExercises.Select(ue => ue.ExerciseId).ToListAsync(), await _context.Exercises.Select(e => e.Id).ToListAsync(), eId => new UserExercise() { ExerciseId = eId, UserId = user.Id });
        _context.AddMissing(await _context.UserExerciseVariations.Select(uev => uev.ExerciseVariationId).ToListAsync(), await _context.ExerciseVariations.Select(ev => ev.Id).ToListAsync(), evId => new UserExerciseVariation() { ExerciseVariationId = evId, UserId = user.Id });
        _context.AddMissing(await _context.UserVariations.Select(uv => uv.VariationId).ToListAsync(), await _context.Variations.Select(v => v.Id).ToListAsync(), vId => new UserVariation() { VariationId = vId, UserId = user.Id });
        await _context.SaveChangesAsync();
