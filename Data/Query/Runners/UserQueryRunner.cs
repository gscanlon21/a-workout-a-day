...
public class UserQueryRunner : BaseQueryRunner
{
    ...
    private async Task<Dictionary<int, List<AllVariationsQueryResults>>> GetAllExercisesVariations(CoreContext context, List<InProgressQueryResults> queryResults)
    {
        ...
        allExercisesVariationsQuery = QueryFilters.FilterSportsFocus(allExercisesVariationsQuery, SportsOptions.SportsFocus, includeNone: true);

        // Further filter down the exercises to those that match our query results.
        allExercisesVariationsQuery = QueryFilters.FilterExercises(allExercisesVariationsQuery, queryResults.ConvertAll(qr => qr.Exercise.Id));
    ...
    private async Task<Dictionary<int, List<PrerequisitesQueryResults>>> GetPrerequisites(CoreContext context, IList<InProgressQueryResults> queryResults)
    {
        ...
        // IncludeNone is true so we check against exercises that our a part of the normal strength training (non-SportsFocus) regimen.
        checkPrerequisitesFromQuery = QueryFilters.FilterSportsFocus(checkPrerequisitesFromQuery, SportsOptions.SportsFocus, includeNone: true);

        // Further filter down the exercises to those that match our query results.
        checkPrerequisitesFromQuery = QueryFilters.FilterExercises(checkPrerequisitesFromQuery, queryResults.SelectMany(qr => qr.Prerequisites.Select(p => p.Id)));
